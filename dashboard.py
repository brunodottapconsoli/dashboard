...
def gerar_grafico_histograma_matplotlib(df_notas):
    fig, ax = plt.subplots()
    ax.hist(df_notas["Nota Final"], bins=20, color='skyblue', edgecolor='black')
...
def gerar_grafico_barra_matplotlib(df_media_criterio):
    ...
    ax.barh(df_media_criterio["Critério"], df_media_criterio["Média_Pontuação"], color='orange')
...
def analytics_aba(candidatos):
    ...
    # Criar faixas de notas (bins)
    bins = list(range(0, 110, 10))  # Ex: 0-10, 10-20, ..., 90-100
...
""")
    ...
    fig_bar = px.bar(df_media_criterio, x="Média_Pontuação", y="Critério",
                     orientation="h", color="Média_Pontuação",
    ...
    fig_corr.update_layout(title="Correlação entre Critérios")
    ...
    # Botão para gerar relatório PDF com matplotlib
...
# --- Função comparar candidatos (mantida simples, opcional) ---
